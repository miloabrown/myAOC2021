...
with open("inputs/sampleinput8.txt", "r") as f:
    data = [row.strip() for row in f]
    outputs = [row.split(" | ")[1].split() for row in data]
    patterns = [row.split(" | ")[0].split() for row in data]
...
def part1():
    return len(
        list(filter(lambda x: str(len(x)) in "2347", chain.from_iterable(outputs)))
    )


# print(f"Part1: {sum(map(part1,outputs))}")
...
def part2():
    ...
    def find_pattern(pattern):
        ...
        digits[6] = list(
            filter(
                lambda x: is_in(
                    add(
                        sub(digits[4], digits[1]),
                        sub(sub(digits[8], digits[4]), digits[7]),
                    ),
                    x,
                ),
                pattern,
        ...
        digits[9] = list(
            filter(
                lambda x: is_in(sub(digits[8], sub(digits[6], digits[5])), x), pattern
            )
        )[0]
        ...
        digits[3] = list(
            filter(
                lambda x: is_in(sub(digits[1], sub(digits[8], digits[6])), x), pattern
            )
        )[0]
    ...
    digit_maps = [find_pattern(row) for row in patterns]
